...
from l2af.nn.layers import SeqAttnMat, GatedEncoding, GatedMultifactorSelfAttnEnc
from l2af.nn.util import sequential_weighted_avg

logger = logging.getLogger(__name__)  # pylint: disable=invalid-name


@Model.register("l2af_3way_ap_modular_binary")
class L2AF3WayAPM(Model):
    ...
    def forward(self,  # type: ignore
        ...
        if self._with_knowledge:
            ...
            encoded_passage = self._variational_dropout(self._pseqlevel_enc(embedded_passage,
                                                                            passage_mask))
        # B * U * H
        if self._shared_rnn:
            encoded_allqa = self._variational_dropout(self._pseqlevel_enc(embedded_all_qa,
                                                                           all_qa_mask))
        else:
            encoded_allqa = self._variational_dropout(self._qaseqlevel_enc(embedded_all_qa,
                                                                           all_qa_mask))
        ...
        if self._with_knowledge and self._is_qdep_penc:
            ...
            q_aware_passage_rep = sequential_weighted_avg(encoded_allqa,
                                                          normalized_attn_mat)  # B * T * H

            q_dep_passage_enc_rnn_input = torch.cat([encoded_passage,
                                                     q_aware_passage_rep], 2)  # B * T * 2H
        ...
        # B * V * H
        if self._shared_rnn:
            encoded_choice = self._variational_dropout(self._pseqlevel_enc(embedded_choice,
                                                                           choice_mask))  # B * V * H
        else:
            encoded_choice = self._variational_dropout(self._cseqlevel_enc(embedded_choice,
                                                                           choice_mask))  # B * V * H

        if self._with_knowledge:
            attn_pq, _ = self._pqaattnmat(encoded_passage, encoded_allqa, all_qa_mask)  # B * T * U
            ...
            agg_prev_qa = norm_attn_pqa.unsqueeze(1).bmm(encoded_allqa).squeeze(1)  # B * H

            attn_pc, _ = self._pcattnmat(encoded_passage,
                                         encoded_choice, choice_mask)  # B * T * V
